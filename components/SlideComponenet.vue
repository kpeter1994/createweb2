...
<template>
  <div class="max-w-7xl mt-16 mx-auto h-[49rem] max-h-[150vh] grid grid-col-1 md:grid-cols-2 lg:grid-cols-3 items-start gap-8 overflow-hidden relative">
    ...
    <button v-if="isPlaying" @click="pause()" class="absolute bottom-3 right-3 text-neutral-800 bg-white bg-opacity-40 border flex justify-center items-center rounded-full w-12 h-12"><i class="pi pi-pause text-neutral-800 text-xl"></i></button>
    <button v-if="!isPlaying" @click="play()" class="absolute bottom-3 right-3 text-neutral-800 bg-white bg-opacity-40 border flex justify-center items-center rounded-full w-12 h-12"><i class="pi pi-play text-neutral-800 text-xl"></i></button>
  </div>
</template>
